...
----What is the total number of seatsavailable for elections in each state
SELECT 
...
ORDER BY Seats_Won DESC
---ADD NEW COLUMN FIELD IN TABLE PARTYWISE_RESULTS TO GET THE PARTY ALLIANZ AS NDA INDIA
---AND OTHER 
...
---WHICH PARTY ALLIANCE (NDA,INDIA, OR OTHER )WONTHE MOST SEATS ACROSS ALL STATES
SELECT 
    p.party_alliance,
...
ORDER BY 
    Seats_Won DESC;
-----WINNING CANDIDAT'S NAME, THEIR PARTY NAME, TOTAL VOTES,AND THE MARGIN OF 
---VICTORY FOR A CONSTITUENCY 
...
-----WHICH PARTIES WON THE MOST SEATS IN STATE, AND HOW MANY SEATS DID EACH 
----PARTY WIN ?
SELECT 
    p.Party,
    COUNT(cr.Constituency_ID) AS Seats_Won
...
ORDER BY 
	...
	--NDA,INDIA, AND OTHER)IN EACH STATE FOR THE INDIA ELECTIONS 2024
	SELECT 
    s.State AS State_Name,
...
WHERE 
	...
	ORDER BY 
	CD.EVM_VOTES DESC;
----WHICH CANDIDATE WON AND WHICH CANDIDATE WAS THE RUNNR-UPIN EACH
